...
local mudrule = require("mud_rule")
...
mudcontroller.del = function(todel)
  return mudrule.delrules(todel)
end

mudcontroller.monitor = function() muddigger.monitor(mudrule.refreshrule) end

mudcontroller.add = function(f_path, mac_addr)
  log.info('Loading file: ', f_path, ' for ', mac_addr )

  status, mud_obj = pcall(mudutil.decode_f, f_path)
  if not status then
